...
public:
  ...
  void Servo2_Init();
  void Servo2_SetAngle(uint8_t angle);   // 0..180
  void Servo2_Nudge(int8_t delta);       // +/− grados
  void Servo2_Step90_Forward();          // 0→90→180→0 pasos de 90°
...
private:
  ...
  // --- Servo2 integration (user mod) ---
  Servo servo2_;
  uint8_t servo2_angle_ = 90;            // arranca centrado
  static const uint8_t SERVO2_PIN = 11;  // pin disponible (PIN_Servo_y), evita conflicto con IR (RECV_PIN = 9)
};
