...
#ifndef DEBUG_IR
#define DEBUG_IR 0   // desactivado para uso normal
#endif
#ifndef DEBUG_IR_HEALTH
#define DEBUG_IR_HEALTH 1   // dejar en 1 por ahora
#endif
...
void ApplicationFunctionSet::ApplicationFunctionSet_Init(void)
{
  ...
  // --- Servo2 integration (user mod) ---
  Servo2_Init();
  // Serial.print("Servo2 on pin: "); Serial.println(SERVO2_PIN); // Ahora pin 11 (evita conflicto con IR pin 9)
...
// --- Servo2 integration (user mod) ---
void ApplicationFunctionSet::Servo2_Init() {
  servo2_.attach(SERVO2_PIN);
  servo2_.write(servo2_angle_);   // 90° neutro
}

void ApplicationFunctionSet::Servo2_SetAngle(uint8_t angle) {
  if (angle > 180) angle = 180;
  servo2_angle_ = angle;
  servo2_.write(servo2_angle_);
}

void ApplicationFunctionSet::Servo2_Nudge(int8_t delta) {
  int16_t next = (int16_t)servo2_angle_ + delta;
  if (next < 0) next = 0;
  if (next > 180) next = 180;
  servo2_angle_ = (uint8_t)next;
  servo2_.write(servo2_angle_);
}

// --- Servo2 integration (user mod) ---
void ApplicationFunctionSet::Servo2_Step90_Forward() {
  // usa servo2_angle_ como estado actual
  uint8_t a = servo2_angle_;
  if      (a < 45)  Servo2_SetAngle(90);
  else if (a < 135) Servo2_SetAngle(180);
  else              Servo2_SetAngle(0);
}
// --- end user mod ---
/*Standby mode*/
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void)
{
  ...
  if (AppIRrecv.DeviceDriverSet_IRrecv_Get(&IRrecv_button /*out*/))
  {
    ...
    #if DEBUG_IR_HEALTH
      Serial.print("IR: 0x");
      Serial.println((unsigned long)IRrecv_button, HEX);
    #endif
  }
  if (true == IRrecv_en)
  {
    ...
    {
      // Botón 8 → mover Servo2 en pasos de 90°
      Servo2_Step90_Forward();
    }
    break;
    ...
    // --- Servo2 integration (user mod) ---
    // Reemplazar 0x00FFA857 y 0x00FF629D por los códigos reales de mi control IR
    case 0x00FFA857:   // ejemplo: CH+
      Servo2_Nudge(+10);
      break;
    case 0x00FF629D:   // ejemplo: CH-
      Servo2_Nudge(-10);
      break;
    // (Opcional) posicionamientos fijos:
    // case 0x00FF22DD: Servo2_SetAngle(30);  break;
    // case 0x00FFC23D: Servo2_SetAngle(150); break;

    default:
      // --- DEBUG_IR (opcional, desactivado por defecto) ---
      #if _DEBUG_IR
        Serial.print("IR_UNKNOWN:");
        Serial.println(IRrecv_button, HEX);
      #endif
      Application_SmartRobotCarxxx0.Functional_Mode = Standby_mode;
      break;
    }
